...
C W-7405-Eng-48.
C
C***BEGIN PROLOGUE  SWEBILU
C***REFER TO  SDASPK
C***DATE WRITTEN   951006   (YYMMDD)
C***REVISION DATE  951010   Converted to single precision.
C***REVISION DATE  951020   Moved DATA statement.
C
...
C
C The DAE system is solved by SDASPK with a sparse approximate Jacobian
C matrix as the preconditioner for the preconditioned Krylov option of
C SDASPK.  The Jacobian is formed by using finite-differences of residual
...
C subroutines --  SSPSETUP, SJACILU and SPSOLILU.
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SETPAR, CINIT, SDASPK, OUTWEB
C
C***END PROLOGUE  SWEBILU
C
      IMPLICIT REAL (A-H,O-Z)
      EXTERNAL RESWEB, SJACILU, SPSOLILU
...
C
     ...
     2                    + (IPREMETH-1)*2*MAXN )
      PARAMETER (LENRW = 91 + 19*MAXN, LENIW  = 40,
...
C
C Set unit number for SDASPK error message output.
      CALL XSETUN(LOUT)
C
C Open output files.
...
C enough work array lengths.
      CALL SSPSETUP (NEQ, LENWP, LENIWP, RPAR, IPAR, IERR,
     .               LWPMIN, LIWPMIN)
      IF (IERR .NE. 0) THEN
         WRITE(LOUT,45) IERR
 45      FORMAT(' Error return from SSPSETUP: IERR = ',i5)
...
C
     ...
     1        IDID, RWORK,LRW, IWORK,LIW, RPAR, IPAR, SJACILU, SPSOLILU)
C
        NST = IWORK(11)
        NRE = IWORK(12)
        NPE = IWORK(13)
...
C
 210  CONTINUE
      ...
      NCFN = IWORK(15)
      NCFL = IWORK(16)
      WRITE(LOUT,220) LNRW,LNIW,NST,NRE,IPAR(30),NPE,NPS,NNI,NLI,
     1               AVLIN,NCFN,NCFL
...
C
      STOP
C------  End of main program for SWEBILU example program ---------------
      END
