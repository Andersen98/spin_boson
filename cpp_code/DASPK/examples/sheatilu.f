...
C
C The routines SJACILU and SPSOLILU that generate and solve the sparse
...
C enough work array lengths.
     ...
     .               LWPMIN, LIWPMIN)
      IF (IERR .NE. 0) THEN
         WRITE(LOUT,15) IERR
 15      FORMAT(' Error return from SSPSETUP: IERR = ',i5)
...
C and we supply the sparse preconditioner routines SJACILU/SPSOLILU.
C
...
C namely subroutine SJACILU, and we must set INFO(15) = 1 to indicate
C this to SDASPK.
C
...
C
C We pass to DDASPK the names SJACILU and SPSOLILU for the JAC and PSOL
...
C subspace involved in the Krylov linear iterative method.
 80   CONTINUE
      ...
      NCFN = IWORK(15)
      NCFL = IWORK(16)
      WRITE (LOUT,90) LRW,LIW,NST,NRE,IPAR(30),NPE,NPS,NNI,NLI,
     1                AVDIM,NCFN,NCFL
...
C
C------  End of main program for SHEATILU example program --------------
      STOP
      END
