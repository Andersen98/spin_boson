...
C                        14 September 1995
C
C The following four subroutines -- SGSET2, GSET1, SRBGJA, SRBGPS --
...
C     SGSET2 to set the mesh and block-grouping data needed later;
C (b) a JAC routine, as prescribed by the DASPK instructions, which
C     calls SRBGJA, and does any other Jacobian-related preprocessing
C     needed for preconditioning; and
C (c) a PSOL routine, as prescribed by the DASPK instructions, which
C     calls SRBGPS for the solution of systems A_R x = b, and does
...
C  * the LINPACK routines SGEFA and SGESL for dense linear sytems, and
C  * the machine constant routine R1MACH for the machine unit roundoff.
C
...
C Also, the use of the SRBGJA/SRBGPS routines in conjunctin with
C DASPK requires that certain mesh-related and block-grouping data
C be set.  This can be done with the call
C     CALL SGSET2 (MX, MY, NS, NSD, NXG, NYG, LID, IWORK)
C The input arguments to SGSET2 are:
...
C
C SGSET2 loads mesh parameters and group data in two COMMON
C blocks, /SRPRE1/ and /RPRE2/, used by these routines.
C Note: the declaration of /RPRE2/ in SGSET2, SRBGJA, and SRBGPS
C uses a parameter MAXM that must be .GE. MAX (MX, MY);
C this must be altered for larger mesh sizes.
C
C SGSET2 also loads the preconditioner work lengths into
C IWORK(27) and IWORK(28), and if LID > 0 it sets the ID array
C in IWORK showing the differential and algebraic components.
C
C SGSET2 generates a rectangular grouping arrangement, with
...
C alter or replace SGSET2 accordingly.
C 
C (b) The JAC routine.
...
C or P_SR, the JAC routine should call SRBGJA for the approximation
C A_R to P_R.  Subroutine SRBGJA generates A_R using difference
C quotients and the block-grouping information.  It then performs an
C LU decomposition of each block, using the LINPACK routine SGEFA.
C
C In terms of the arguments passed to JAC by DASPK, the call to
C SRBGJA should have the form
C     CALL SRBGJA (T, U, R0, RBLOCK, WK, REWT, CJ, WP, IWP, IER)
...
C loop over mesh points in SRBGJA.
C
C To use SRBGJA, the user must provide the following subroutine,
C which SRBGJA calls to obtain individual blocks of R:
...
C should call SRBGPS for the solution of A_R.  Subroutine SRBGPS
C solves a linear system A_R x = b, using the LINPACK backsolve
C routine SGESL.  In terms of the arguments passed to PSOL by DASPK,
C the call to SRBGPS should have the form
C     CALL SRBGPS (B, WP, IWP)
C SRBGPS overwrites the B array (containing b) with the solution x.
C
C-----------------------------------------------------------------------
C
C References
...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE SGSET2 (MX, MY, NS, NSD, NXG, NYG, LID, IWORK)
C***BEGIN PROLOGUE  SGSET2
...
C SRBGJA and SRBGPS routines, assuming a 2-D rectangular problem with
C uniform rectangular block-grouping.  Given the mesh and group
C parameters, it loads the COMMON blocks /SRPRE1/ and /RPRE2/, and the
...
C            UROUND = R1MACH(4) generates the unit roundoff.
...
C be altered consistently in subroutines SRBGJA and SRBGPS.
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   R1MACH, GSET1
C
C***END PROLOGUE  SGSET2
C
      IMPLICIT REAL (A-H,O-Z)
      DIMENSION IWORK(*)
      PARAMETER (MAXM = 50)
      COMMON /SRPRE1/ SRUR, MP, MPD, MPSQ, MESHX, MESHY, MXMP
...
C
      DO 30 IG = 1,NGM1
 30     JR(IG) = 0.5E0 + (REAL(IG) - 0.5E0)*REAL(MPER)
      JR(NG) = 0.5E0*REAL(1 + NGM1*MPER + M)
C
      RETURN
C------------  End of Subroutine GSET1  --------------------------------
      END

      SUBROUTINE SRBGJA (T, U, R0, RBLOCK, R1, REWT, CJ, BD, IPBD, IER)
C***BEGIN PROLOGUE  SRBGJA
...
C It calls SGEFA to do LU decomposition of each diagonal block.
C The computation of the diagonal blocks uses the mesh and grouping
C information in the COMMON blocks /SRPRE1/ and /RPRE2/.  One block
...
C   RBLOCK, SGEFA
C
C***END PROLOGUE  SRBGJA
C
      IMPLICIT REAL (A-H,O-Z)
      EXTERNAL RES, RBLOCK
      DIMENSION U(*), R0(*), R1(*), REWT(*), BD(*), IPBD(*)
C
C The declaration of /RPRE2/ below must agree with that in SGSET2.
      PARAMETER (MAXM = 50)
      COMMON /SRPRE1/ SRUR, MP, MPD, MPSQ, MESHX, MESHY, MXMP
      COMMON /RPRE2/ NGX, NGY, NGRP, JGX(MAXM+1), JGY(MAXM+1),
     1               JIGX(MAXM), JIGY(MAXM), JXR(MAXM), JYR(MAXM)
...
C Add matrix CJ * I_d, and do LU decomposition on blocks. --------------
 ...
 80     CONTINUE
 90   RETURN
C------------  End of Subroutine SRBGJA  -------------------------------
      END

      SUBROUTINE  SRBGPS (B, BD, IPBD)
C***BEGIN PROLOGUE  SRBGPS
C***DATE WRITTEN   950914   (YYMMDD)
C***REVISION DATE  950918   Converted to single precision.
C
C***AUTHORS  A. C. Hindmarsh
...
C of the diagonal blocks computed in SRBGJA, and the mesh and
C block-grouping data in the COMMON blocks /SRPRE1/ and /RPRE2/.
C Here BD is the RWORK segment WP, and IPBD is the IWORK segment IWP.
...
C
      ...
      DO 20 JY = 1,MESHY
        ...
        DO 10 JX = 1,MESHX
          ...
          IBD = 1 + IGM1*MPSQ
          IIP = 1 + IGM1*MP
          CALL SGESL (BD(IBD), MP, MP, IPBD(IIP), B(IB), 0)
          IB = IB + MP
 10       CONTINUE
 20     CONTINUE
C
      RETURN
C------------  End of Subroutine SRBGPS  -------------------------------
      END
