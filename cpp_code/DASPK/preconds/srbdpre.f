...
C                        14 September 1995
C
C The following three subroutines -- SMSET2, SRBDJA, SRBDPS --
...
C     SMSET2 to set mesh data and mesh-related DASPK inputs;
C (b) a JAC routine, as prescribed by the DASPK instructions, which
C     calls SRBDJA, and does any other Jacobian-related preprocessing
C     needed for preconditioning; and
C (c) a PSOL routine, as prescribed by the DASPK instructions, which
C     calls SRBDPS for the solution of systems A_R x = b, and does
...
C
C Also, the use of the SRBDJA/SRBDPS routines in conjunction with
C DASPK requires that certain mesh-related data be set.  This can be
C done with the call
C     CALL SMSET2 (MX, MY, NS, NSD, LID, IWORK)
C The input arguments to SMSET2 are:
...
C SMSET2 loads mesh data in a COMMON block /SRPRE1/ used by the
C SRBDJA/SRBDPS routines.
C
C SMSET2 also loads the preconditioner work lengths into
...
C or P_SR, the JAC routine should call SRBDJA for the approximation
C A_R to P_R.  Subroutine SRBDJA generates A_R using difference
C quotients.  It then performs an LU decomposition of each block,
C using the LINPACK routine SGEFA.
C
C In terms of the arguments passed to JAC by DASPK, the call to
C SRBDJA should have the form
C     CALL SRBDJA (T, U, R0, RBLOCK, WK, REWT, CJ, WP, IWP, IER)
...
C loop over mesh points in SRBDJA.
C
C To use SRBDJA, the user must provide the following subroutine,
C which SRBDJA calls to obtain individual blocks of R:
...
C should call SRBDPS for the solution of A_R.  Subroutine SRBDPS
...
C the call to SRBDPS should have the form
C     CALL SRBDPS (B, WP, IWP)
C SRBDPS overwrites the B array (containing b) with the solution x.
C
C-----------------------------------------------------------------------
C
C References
...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE SMSET2 (MX, MY, NS, NSD, LID, IWORK)
C***BEGIN PROLOGUE  SMSET2
...
C SRBDJA and SRBDPS, assuming a 2-D rectangular problem.
C Given the mesh parameters, it loads the COMMON block /SRPRE1/,
...
C
      RETURN
C------------  End of Subroutine SMSET2  -------------------------------
      END

      SUBROUTINE SRBDJA (T, U, R0, RBLOCK, R1, REWT, CJ, BD, IPBD, IER)
C***BEGIN PROLOGUE  SRBDJA
...
C the COMMON block /SRPRE1/.  One block per spatial point is computed.
...
C Add matrix CJ * I_d, and do LU decomposition on blocks. --------------
 ...
 80     CONTINUE
 90   RETURN
C------------  End of Subroutine SRBDJA  -------------------------------
      END

      SUBROUTINE  SRBDPS (B, BD, IPBD)
C***BEGIN PROLOGUE  SRBDPS
C***DATE WRITTEN   950914   (YYMMDD)
C***REVISION DATE  950918   Converted to single precision.
C
C***AUTHORS  A. C. Hindmarsh
...
C of the diagonal blocks computed in SRBDJA, and mesh parameters
C in the COMMON block /SRPRE1/.
C Here BD is the RWORK segment WP, and IPBD is the IWORK segment IWP.
...
C
      ...
      DO 20 JY = 1,MESHY
        DO 10 JX = 1,MESHX
          CALL SGESL (BD(IBD), MP, MP, IPBD(IB), B(IB), 0)
          IB = IB + MP
          IBD = IBD + MPSQ
 10       CONTINUE
 20     CONTINUE
